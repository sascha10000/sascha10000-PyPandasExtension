...
class DataEnhancer:
  ...
  def process_data(self, row_operations: Callable[['DataEnhancer', pd.DataFrame], List[RowOperation]], store_to_file: bool = True, limit=-1) -> pd.DataFrame:
    ...
    for file in self.files:
      ...
      for amount_chunks, chunk in enumerate(reader):
        ...
        processed = self.do_row_operations(chunk, row_operations(self, chunk))
  ...
  @staticmethod
  def do_row_operations(df: pd.DataFrame, row_operations: List[RowOperation]) -> pd.DataFrame:
    row_operations = SequencedRowOperations(row_operations)
